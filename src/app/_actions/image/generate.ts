...
// Initialize Together lazily to avoid crashing when the API key is not set
...
export async function generateImageAction(
  ...
  try {
    ...
    // Ensure Together is configured
    const together = getTogetherClient();
    if (!together) {
      return {
        success: false,
        error:
          "Image generation is disabled: set TOGETHER_AI_API_KEY in .env to enable.",
      } as const;
    }
