...
export async function POST(req: Request) {
  try {
    ...
    const hasSearchEnabled = process.env.TAVILY_API_KEY && process.env.TAVILY_API_KEY.trim() !== "";
    ...
    // Only add web search tool if API key is configured
    if (hasSearchEnabled) {
      streamOptions.tools = {
        webSearch: search_tool,
      };
      streamOptions.maxSteps = 5; // Allow up to 5 tool calls
      streamOptions.toolChoice = "auto"; // Let the model decide when to use tools
      console.log("✅ Web search enabled for outline generation");
    } else {
      console.warn("⚠️ Web search disabled - TAVILY_API_KEY not configured");
    }
