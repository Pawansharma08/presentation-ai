...
export function ModelPicker({
  ...
  useEffect(() => {
    if (!hasRestoredFromStorage.current) {
      ...
      if (savedModel) {
        ...
        setModelProvider(
          savedModel.modelProvider as "openai" | "ollama" | "lmstudio" | "bedrock",
        );
  ...
  const handleModelChange = (value: string) => {
    ...
    if (value === "openai") {
      ...
      setModelProvider("bedrock");
      setModelId("");
      setSelectedModel("bedrock", "");
      console.log("Saved to localStorage: bedrock, ''");
          ...
          <SelectGroup>
            ...
            <SelectItem value="bedrock">
              <div className="flex items-center gap-3">
                <Bot className="h-4 w-4 flex-shrink-0" />
                <div className="flex flex-col min-w-0">
                  <span className="truncate text-sm">Bedrock Claude 3 Sonnet</span>
                  <span className="text-xs text-muted-foreground truncate">
                    AWS Bedrock model
                  </span>
                </div>
